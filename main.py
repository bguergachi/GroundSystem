...
class Display:
    ...
    def __init__(self, master):
        ...
        #self.__master.after(200,lambda: self.__init__(master))
    ...
    def statusBar(self):
        ...
        mapCoordinates = Frame(status, bg=statusBG, height=70, width=height / 3 - 4)
        ...
        statusStopWatch = Frame(status, bg=statusBG, height=70, width=height / 3 - 4)
        ...
        altitudePressure = Frame(status, bg=statusBG, height=70, width=height / 3 - 4)
        ...
        img = Label(mapCoordinates,image = render,bg=statusBG)
        img.image = render
        ...
        stopWatch = Label(statusStopWatch, text = self.getStopWatch(),bg = statusBG,fg="black")
        stopWatch.config(font=("times",20))
        stopWatch.pack(side=TOP,anchor=N)

        #Paint status indication for separation
        statusFlag = Label(statusStopWatch,text="status",bg = statusBG,fg = self.getStatusFlag())
        statusFlag.config(font=("times",15))
        statusFlag.pack(side=TOP,anchor=S)

        #Paint label of altitude or pressure value
        altitude_pressure = Label(altitudePressure, text=self.getAltitudePressureData() + "m", bg = statusBG)
        altitude_pressure.config(font=("times,20"))
        altitude_pressure.pack(side = TOP,anchor=N)

        #Paint descriptive label of data
        title_of_data = Label(altitudePressure,text=self.getAltitudePressureTitle(),bg = statusBG)
