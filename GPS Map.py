...
class Map:
    ...
    def frame_dimensions(self):
        self.__latitude_top = self.map_side['top']
        self.__latitude_bot = self.map_side['bot']
        self.__latitude_length = self.__latitude_top - self.__latitude_bot

        self.__longitude_right = self.map_side['right']
        self.__longitude_left = self.map_side['left']
        self.__longitude_length = self.__longitude_left - self.__longitude_right
        return [self.__latitude_length, self.__longitude_length]

    # Random test coordinates
    def getRandomNumber(self):
        self.__rand_lat = round(random.uniform(self.__latitude_bot, self.__latitude_top), 6)
        self.__rand_long = round(random.uniform(self.__longitude_right, self.__longitude_left), 6) * (-1)
    ...
    def display_map(self):
        self.__load_map.paste(load_crosshair, (self.__pixel_integer[1] - int(crosshair_size[0] / 2), self.__pixel_integer[0] - int(crosshair_size[1] / 2)), load_crosshair)  # Positions and blends Crosshair with map
        map = ImageTk.PhotoImage(self.__load_map)
        crosshair = ImageTk.PhotoImage(load_crosshair)
        label_map = Label(self.frame, image=map)
        label_map.pack()
    ...
    def choose_map(self):
        if self.__random_coordinate[0] > self.__latitude_top or self.__random_coordinate[0] < self.__latitude_bot:
            ...
            self.map_side = self.largemap_side.copy()
            self.__load_map = load_largemap
        elif self.__random_coordinate[1] * (-1) > self.__longitude_left or self.__random_coordinate[1] * (-1) < self.__longitude_right:
            tkinter.messagebox.showwarning('Warning', 'Rocket has left the competition area!!!')
            print("Rocket has left competition area!!!")
            self.map_side = self.largemap_side.copy()
            self.__load_map = load_largemap
        elif self.__random_coordinate[0] < self.__latitude_top or self.__random_coordinate[0] > self.__latitude_bot:
            self.map_side = self.smallmap_side.copy()
            self.__load_map = load_smallmap
        elif self.__random_coordinate[1] * (-1) < self.__longitude_left or self.__random_coordinate[1] * (-1) > self.__longitude_right:
            self.map_side = self.smallmap_side.copy()
            self.__load_map = load_smallmap
